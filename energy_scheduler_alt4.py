...
class Alt4Solver:
    ...
    def solve(
        ...
        # overshoot[t, n] = [τ_t − s_n]_+   (shape (T, N), in seconds)
        overshoot = tau_candidates[:, None] - s_n[None, :]      # (T, N)
        np.maximum(overshoot, 0.0, out=overshoot)
        # φ[t, n] = max(1 − μ · overshoot, φ_min)
        phi_mat = np.maximum(1.0 - mu * overshoot, phi_min)     # (T, N)
        u_mat = r_n_vec[None, :] * phi_mat                      # (T, N)
        ...
        for f in freqs:
            ...
            # Per-request t_{n,f} in SECONDS
            denom_t = np.where(is_pf, float(f), f_alpha)
            t_q_s = (t_num / denom_t) / 1000.0                   # (N,)

            # Per-request "energy cost" in JOULES (W · s)
            energy_cost = beta * P_f * t_q_s                     # (N,)

            # v_n(τ, f) = u_n(τ) − energy_cost(n, f)             # (T, N)
            ...
            for M in masks:
                ...
                ratio_mat = np.where(
                    elig_mat, v_mat / denom_persist[None, :], -np.inf
                )
                order_mat = np.argsort(-ratio_mat, axis=1, kind="stable")  # (T, N)

                # Per-row gathered arrays.  For 1-D source arrays, fancy
                # indexing source[order_mat] returns shape (T, N) directly.
                gather_v = np.take_along_axis(v_mat, order_mat, axis=1)
                gather_u = np.take_along_axis(u_mat, order_mat, axis=1)
                gather_t = t_q_s[order_mat]                       # (T, N)
                gather_tok = tok_arr[order_mat].astype(np.float64)
                ...
                elig_f = gather_elig.astype(np.float64)
