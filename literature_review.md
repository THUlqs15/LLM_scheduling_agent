...
## 3. SLOs-Serve: Optimized Serving of Multi-SLO LLMs

### 研究问题

SLOs-Serve 面向更复杂的 multi-stage LLM requests：不同应用可能有不同 prefill SLO、decode SLO，reasoning 模型还可能有 thinking stage。已有 co-located scheduler 往往只优化某个 stage，PD disaggregation 又依赖固定 prefill/decode 设备比例，难以适应动态 workload。

研究问题是：如何在 multi-stage、multi-SLO、bursty arrivals 和 multi-replica 场景下，决定哪些请求应被保证服务，以及每个 batch 中 token 应该如何分配。

### 核心算法

**1. Soft admission control**

SLOs-Serve 不贪心接受所有请求。它周期性调用 scheduler，对 running requests 和 new requests 做联合规划：

- running requests 被 forced admission，因为系统已经承诺它们的 SLO。
- new requests 中只接受可在未来 batch plan 中满足所有 stage SLO 的子集。
- declined requests 不一定硬拒绝，可进入 best-effort tier，或在 multi-replica 中路由到其他 replica。

因此系统维护一个 invariant：

> admitted requests 的每个 stage SLO 都应被满足。

**2. Multi-SLO dynamic programming**

假设请求 $R_i$ 按 prefill deadline $pDDL_i$ 排序。每个请求有：

- arrival time $t_i$
- prompt length $p_i$
- memory requirement $m_i$
- prefill deadline $pDDL_i$
- decode TPOT SLO tier
- value $v_i$
...
表示：考虑 prefill deadline 最早的前 $i$ 个请求，在使用 $m$ memory units、到 $R_i$ 的 prefill deadline 时留下 $pb$ prefill budget、已接受 $n$ 个请求时，最大可获得 value。
...
\max_j
\left\{
DP[j,m-m_i,pb+p_i-\Delta_{i,j}^{n-1}pb,n-1]+v_i
\right\},
$$

约束为 $pDDL_j < pDDL_i$、$\Delta pb\ge 0$、$m\ge m_i$。这里：

$$
\Delta_{i,j}^{n}pb = PB^*(pDDL_i-pDDL_j,n),
$$

而 $PB^*(t,n)$ 是在时间区间 $t$ 内、保证 $n$ 个 decode 请求 SLO 的前提下，能产生的最大 prefill token budget：

$$
PB^*(t,n)=
\max_{\text{partial batches}}
\sum_{b\in \text{partial batches}} b.\text{prefillBudget}
$$

subject to decode SLO constraints。

直觉是：decode SLO 是必须先满足的“刚性约束”，剩余 token budget 才能给 prefill。若某个请求的 prefill deadline 到来前累计 prefill budget 非负，则这个请求的 prefill SLO 可被满足。

**3. Batch formation with dynamic size tuning**

在 autoregressive decode 下，先取当前 running decode 中最紧的 TPOT：

$$
t_0=\min_{r\in Reqs_{Decoding}} r.TPOT.
$$

每个 $t_0$ 时间 slot 构造一个 batch：

- batch capacity 由 performance model 的 $\text{time2bs}(t_0)$ 给出。
- 用 earliest scheduled deadline queue 给 decode requests 分配 token。
- 每分配一个 decode token，prefill budget 减一。
- 剩下的 budget 给 prefill requests。

相对于 Sarathi-Serve 的固定 batch-size cap，SLOs-Serve 会根据当前 decode set 动态调 batch size。
